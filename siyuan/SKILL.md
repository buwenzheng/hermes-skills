...
- **配置**：通过 `~/.hermes/skills/siyuan/scripts/siyuan_config.json` 或环境变量配置
...
脚本默认启用本地缓存，缓存文件：`~/.hermes/skills/siyuan/scripts/siyuan_cache.json`

| 数据 | TTL | 说明 |
|------|-----|------|
| `notebooks` | 5 分钟 | 笔记本列表 |
| `doc_tree:<id>` | 1 小时 | 各笔记本文档树 |
...
### `cmd_get_ids_by_hpath` 不是 CLI 命令
它是 Python API 函数 `cmd_get_ids_by_hpath(config, hpath)`，但未在 CLI 层暴露。搜索文档用 `search_doc <关键词> [笔记本ID]` 代替。
